# PSDL 0.4 — Packet Schema Definition Language

PSDL is a small declarative schema for describing the bit-level layout of
...
This document describes PSDL 0.4. The JSON serialization is normative;
the in-memory TypeScript shape lives at
`web/lib/psdl/types.ts` and the JSON Schema at
`schemas/psdl.schema.json`.

> **What's new in 0.4.** Four additive primitives push PSDL past
...
- **Expressions, not closures.** Variable-length and conditional layout
  ...
  No JavaScript closures cross the wire; PSDL is reproducible across
  toolchains.
...
- **N+M, not N×M.** Every format converts only to/from PSDL; pairwise
...
not by the schema. PSDL 0.3 names three encodings:
...
is consumed as the concrete bit count. This keeps PSDL reproducible:
...
```ts
type TlvInstancePsdl = { kind: number; extras?: Record<string, number> };
type ChainInstancePsdl = { proto: number; extras?: Record<string, number> };
...
present, `psdlToRenderer` lifts each entry onto the renderer mirror's
...
**Why this isn't purely presentational.** PSDL's design principle is
...
serialises the cache. We picked PSDL-side persistence so the URL
...
encryption is in play. PSDL 0.3 models this with a `viewMode` parameter
...
`viewMode` is **not** part of the on-disk PSDL schema. It's UI state.
...
PSDL 0.4 adds an optional `byteOrder` field on `Field` itself. When
...
The full schema lives at `schemas/psdl.schema.json`.

---

## Worked example: IPv4

The IPv4 header is the canonical PSDL example because it exercises
...
nibble is an opaque numeric (PSDL's `int` works just as well; `bits`
...
because PSDL's runtime model is "everything is a field tree", and this
...
There's no PSDL magic for "TLV"; a TLV is just `Repeat<Switch>` with a
...
from PSDL — there is no separate runtime resolver. For the React editing
components (TLV editor, chain editor, detail panel) PSDL is lowered to a
renderer-shaped Packet by `web/lib/psdl/psdl-to-renderer.ts`, which
...
exposes `rendererToPsdl` for the inverse lift (used by the import/export
drawer). The format hub itself is three files:

- `web/lib/formats/json.ts` — `toJson(psdlPacket, env)` /
  `fromJson(text)`.
- `web/lib/formats/rfc-ascii.ts` — `toAscii(psdlPacket, env, opts?)`
  (uses PSDL's normalize internally; `opts.viewMode` selects wire vs
...
- **Kaitai (.ksy)** — Optional becomes `if: <kaitai-expr>` when the
  ...
  a `# psdl-only: optional …` comment. `berLength` is emitted as a
  `u1` placeholder plus a `# psdl-only: berLength` comment. `peek` is
...
- **Kaitai (.ksy)** — Kaitai has no native notion of either Encrypted
  ...
  (`# psdl-only: encrypted block …` and `# psdl-only: varint …`) and
  the importer recognises no inverse construct; round-tripping a 0.3
  packet through `.ksy` is lossy by design.

## Kaitai interop

PSDL ships a light import/export bridge to [Kaitai Struct](https://kaitai.io/)
`.ksy` YAML files in `web/lib/formats/ksy.ts`. The bridge is read-mostly:
the importer prioritises producing a useful PSDL packet from real-world
...
anything PSDL-specific as YAML comments prefixed `# psdl-only:`.

### Supported (import)

| Kaitai construct                     | PSDL mapping                                         |
| ------------------------------------ | ---------------------------------------------------- |
...
| `process: zlib \| xor \| rotate`       | won't-fix | warned and dropped (PSDL has no transform pipeline)                                                                                                              |
...
needed) `types:`. PSDL-only features survive as comments at the top of
the file: `# psdl-only: ...` lines tag dropped `category` tokens, dropped
...
Built-in presets live as YAML in `data/presets/<key>.psdl.yaml`. They are
validated against `schemas/psdl.schema.json` and compiled to
`web/lib/psdl/presets.generated.ts` by `web/scripts/build-presets.ts` at
build time (wired through the `prebuild` and `pretest` npm hooks).

To add a new preset:

1. Drop a file into `data/presets/`, e.g. `mything.psdl.yaml`. The
...
The generated file (`web/lib/psdl/presets.generated.ts`) is gitignored;
do not edit it by hand and do not commit it.

## Renderer interpretation

PSDL is intentionally semantic-only — the schema declares structure and
...
The rewrite happens in `web/lib/psdl/psdl-to-renderer/apply-tlv.ts`. It
runs only at layout time — `NormalizedField[]` and the on-disk PSDL are
unchanged, so a roundtrip through JSON / share-URL stays canonical.

### Override surfaces

Editing affordances are derived from PSDL primitives, not declared in
the schema:
...
widgets. PSDL doesn't need a `ui:` field — the renderer picks an
affordance based on the structure alone.

## Out of scope

- Actual decryption — PSDL 0.3 models the _shape_ of an encrypted
