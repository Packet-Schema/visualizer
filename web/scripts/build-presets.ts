// PSML 0.2 — runtime preset build step.
//
// Reads the Typst source (`data/presets.typ`), runs it through our minimal
// Typst parser, validates each preset, and emits a TypeScript module
// (`lib/psml/runtime-presets.ts`) that exports the typed PRESETS registry
// plus PRESET_KEYS.
//
// Re-attaches `toBits` lazily at runtime via `lib/psml/runtime-resolver.ts`
// so the generated file stays pure JSON-like.
...
const outPath = resolve(root, "lib/psml/runtime-presets.ts");
const worksheetTypstPath = resolve(root, "data/worksheet.typ");
const worksheetOutPath = resolve(root, "lib/worksheet-template.ts");
...
function main(): void {
  ...
  const header = `// PSML 0.2 — runtime preset registry.
// AUTO-GENERATED by scripts/build-presets.ts.
// Edit data/presets.typ and run \`npm run build:data\` instead.

import { attachToBits } from "./runtime-resolver";
import type { Packet, PacketRegistry } from "./runtime-types";
...
`;
  ...
  // Inline the worksheet Typst template so the runtime worksheet generator
  // (lib/worksheet-typst.ts) doesn't need to fetch it. Next.js with static
  // export doesn't ship arbitrary asset files; a TS module is the safest path.
  const worksheetSource = readFileSync(worksheetTypstPath, "utf8");
  const worksheetModule = `// AUTO-GENERATED by scripts/build-presets.ts from data/worksheet.typ.
// Edit data/worksheet.typ and run \`npm run build:data\` instead.

export const WORKSHEET_TYPST_SOURCE = ${JSON.stringify(worksheetSource)};
`;
  writeFileSync(worksheetOutPath, worksheetModule, "utf8");
  console.log(`build-presets: wrote ${worksheetOutPath}`);
}
