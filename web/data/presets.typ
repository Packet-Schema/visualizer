...
// `formula:` strings that are interpreted by the resolver via
// TO_BITS_REGISTRY (see lib/packet-resolver.ts). Likewise, count-driven TLV
// catalog entries declare a `fieldsFormula:` resolved via TLV_FIELDS_REGISTRY.
...
#let ipv4 = (
  ...
  fields: (
    ...
    (id: "options", name: "Options", color: "amber", category: "variable",
      ...
      tlv: (
        catalog: (
          (kind: 0, name: "EOL", bits: 8,
            description: "End of Option List — final option; remaining option bytes are padding. [RFC 791]",
            fields: (
              (id: "type", name: "Type=0", bits: 8),
            )),
          (kind: 1, name: "NOP", bits: 8,
            description: "No Operation — single-byte padding used to align the next option on a 4-byte boundary. [RFC 791]",
            fields: (
              (id: "type", name: "Type=1", bits: 8),
            )),
          (kind: 7, name: "Record Route",
            description: "Record Route — each router along the path inserts its IP address. [RFC 791]",
            defaultExtras: (count: 3),
            variableCount: (key: "count", min: 1, max: 9, label: "Address slots"),
            fieldsFormula: "ipv4_record_route"),
          (kind: 131, name: "Loose Source Route",
            description: "Loose Source and Record Route — sender lists intermediate routers but other hops are allowed in between. [RFC 791]",
            defaultExtras: (count: 2),
            variableCount: (key: "count", min: 1, max: 9, label: "Address slots"),
            fieldsFormula: "ipv4_loose_source_route"),
          (kind: 137, name: "Strict Source Route",
            description: "Strict Source and Record Route — packet must traverse exactly the listed routers, in order. [RFC 791]",
            defaultExtras: (count: 2),
            variableCount: (key: "count", min: 1, max: 9, label: "Address slots"),
            fieldsFormula: "ipv4_strict_source_route"),
          (kind: 68, name: "Timestamp",
            description: "Internet Timestamp — routers stamp their clock as the packet transits. [RFC 791]",
            defaultExtras: (count: 2),
            variableCount: (key: "count", min: 1, max: 9, label: "Timestamp slots"),
            fieldsFormula: "ipv4_timestamp"),
        ),
        instances: (),
        padToBoundary: 32,
  ...
  fields: (
    ...
    (id: "options", name: "Options", color: "amber", category: "variable",
      ...
      tlv: (
        catalog: (
          (kind: 0, name: "EOL", bits: 8,
            description: "End of Option List — marks the final option; remainder padded with zero bytes. [RFC 9293]",
            fields: (
              (id: "kind", name: "Kind=0", bits: 8),
            )),
          (kind: 1, name: "NOP", bits: 8,
            description: "No Operation — single-byte padding so the next option can align on a 4-byte boundary. [RFC 9293]",
            fields: (
              (id: "kind", name: "Kind=1", bits: 8),
            )),
          (kind: 2, name: "MSS",
            description: "Maximum Segment Size — sent in SYN to advertise the largest payload this side can accept. [RFC 9293]",
            fields: (
              (id: "kind", name: "Kind=2", bits: 8),
              (id: "length", name: "Len=4", bits: 8),
              (id: "mss", name: "MSS", bits: 16),
            )),
          (kind: 3, name: "Window Scale",
            description: "Shift count for the Window field — multiplies advertised window by 2^N, allowing >64KB receive windows. [RFC 7323]",
            fields: (
          ...
          (kind: 4, name: "SACK Permitted",
            description: "Sender supports Selective Acknowledgments — usually negotiated in SYN/SYN-ACK. [RFC 2018]",
            fields: (
              (id: "kind", name: "Kind=4", bits: 8),
              (id: "length", name: "Len=2", bits: 8),
            )),
          (kind: 5, name: "SACK",
            description: "Selective ACK blocks — reports out-of-order data so the sender retransmits only what was lost. Each block is 8 bytes (left edge + right edge). [RFC 2018]",
            fields: (
          ...
          (kind: 8, name: "Timestamps",
            description: "Sender's TS Value plus echoed peer TS Value — used for RTT measurement and PAWS (Protect Against Wrapped Sequence numbers). [RFC 7323]",
            fields: (
...
#let ipv6 = (
  ...
  fields: (
    ...
    (id: "nextHeader", name: "Next Header", bits: 8, color: "teal", category: "type",
      ...
      chainCatalog: (
        (proto: 0, name: "Hop-by-Hop Options",
          description: "Hop-by-Hop Options header — examined by every node along the path; first extension header if present. [RFC 8200 §4.3]",
          fields: (
            (id: "nextHeader", name: "Next Header", bits: 8),
            (id: "hdrExtLen", name: "Hdr Ext Len", bits: 8),
            (id: "options", name: "Options + padding", bits: 48),
          )),
        (proto: 43, name: "Routing",
          description: "Routing header — directs the packet through a list of intermediate nodes (deprecated Type 0 source-route; Type 4 is Segment Routing). [RFC 8200 §4.4]",
          fields: (
            (id: "nextHeader", name: "Next Header", bits: 8),
            (id: "hdrExtLen", name: "Hdr Ext Len", bits: 8),
        ...
        (proto: 44, name: "Fragment",
          description: "Fragment header — IPv6 fragments are produced only by the source host, not by intermediate routers. [RFC 8200 §4.5]",
          fields: (
            (id: "nextHeader", name: "Next Header", bits: 8),
        ...
        (proto: 60, name: "Destination Options",
          description: "Destination Options header — examined only by the packet's final destination(s). [RFC 8200 §4.6]",
          fields: (
            (id: "nextHeader", name: "Next Header", bits: 8),
            (id: "hdrExtLen", name: "Hdr Ext Len", bits: 8),
            (id: "options", name: "Options + padding", bits: 48),
        ...
        (proto: 51, name: "AH",
          description: "Authentication Header (IPsec) — provides integrity/authentication for the packet. [RFC 4302]",
          fields: (
            (id: "nextHeader", name: "Next Header", bits: 8),
        ...
        (proto: 50, name: "ESP",
          description: "Encapsulating Security Payload (IPsec) — provides confidentiality and integrity by encrypting the payload. [RFC 4303]",
          fields: (
            (id: "spi", name: "SPI", bits: 32),
            (id: "seq", name: "Sequence Number", bits: 32),
...
#let tlsClientHello = (
  ...
  fields: (
    ...
    (id: "extensions", name: "Extensions", color: "amber", category: "variable",
      ...
      formula: "tls_extensions",
      description: "Sequence of TLS extensions (Type, Length, Value). Each extension carries its own internal layout. [RFC 8446 §4.2]",
      tlv: (
        catalog: (
          (kind: 0, name: "server_name (SNI)",
            description: "Server Name Indication — tells the server which virtual host the client wants so it can pick the right certificate. [RFC 6066]",
            fields: (
          ...
          (kind: 16, name: "ALPN",
            description: "Application-Layer Protocol Negotiation — client lists protocols it can speak (e.g. h2, http/1.1). [RFC 7301]",
            fields: (
          ...
          (kind: 10, name: "supported_groups",
            description: "Lists the (EC)DHE groups the client supports for key exchange — e.g. x25519, secp256r1. [RFC 8446 §4.2.7]",
            fields: (
          ...
          (kind: 43, name: "supported_versions",
            description: "Lists TLS versions the client supports — this is where TLS 1.3 (0x0304) is actually advertised. [RFC 8446 §4.2.1]",
            fields: (
          ...
          (kind: 51, name: "key_share",
            description: "Carries the client's (EC)DHE public key share(s); enables a 1-RTT handshake. [RFC 8446 §4.2.8]",
            fields: (
