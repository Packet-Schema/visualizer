// PSML 0.4 — Optional container tests.
//
// Verifies:
...
describe("validatePsmlPacket — Optional", () => {
  it("accepts a well-formed optional", () => {
    expect(() => validatePsmlPacket(mkPacket(mkOptional()))).not.toThrow();
  });
  ...
  it("rejects an inner field with no type", () => {
    ...
    expect(() => validatePsmlPacket(mkPacket(bad))).toThrow(/missing a type/);
  });
});
