// PSDL 0.3 — Encrypted container tests.
//
...
import type { Encrypted, Packet, Struct } from "../../lib/psdl/types";
...
describe("validatePsdlPacket — Encrypted", () => {
  it("accepts a well-formed encrypted container", () => {
    expect(() => validatePsdlPacket(mkPacket(mkEncrypted()))).not.toThrow();
  });

  it("rejects empty plaintext", () => {
    const enc = mkEncrypted({ plaintext: { id: "p", fields: [] } });
    expect(() => validatePsdlPacket(mkPacket(enc))).toThrow(
      /at least one field/,
    );
  });
  ...
  it("rejects an empty contextNote", () => {
    ...
    expect(() => validatePsdlPacket(mkPacket(enc))).toThrow(/contextNote/);
  });
  ...
  it("rejects a non-array headerProtected", () => {
    ...
    expect(() => validatePsdlPacket(mkPacket(enc))).toThrow(/must be an array/);
  });
});
