...
function mkPacket(name: string): PsdlPacket {
  return {
    name,
    rowBits: 32,
...
describe("listCustomPresets", () => {
  ...
  it("drops entries that fail PSDL validation (e.g. missing name)", () => {
    // `readRaw` runs `validatePsdlPacket` on each entry now so a third
