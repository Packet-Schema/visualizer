// PSML AST 内の ref 名を全部集める walker。
//
// 同等の walker が `components/packet-viewer/PacketViewer.tsx` 内に存在する
// が、editor 側はそこに依存させたくない (PacketViewer は controllers / UI
// state を抱えていて重い) ため、純粋関数だけここに切り出す。 PacketViewer
// 側がこちらに寄ってくる reorg は別 PR で。

import type { Expr, PsmlPacket } from "./types";
...
/**
 * PsmlPacket 内で参照される ref フィールド名 (= Expr.ref.field) を漏れなく
 * 集める。 未 seed の ref は呼び出し側で 0 に倒すなどの fallback に使う。
 */
