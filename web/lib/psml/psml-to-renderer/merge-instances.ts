...
// edits. The PSML extensions (`Repeat.instances` / `Repeat.chainInstances`
// in 750a76c) made the state representable in PSML; this helper actually
// merges them on the way out so the export paths reflect what's on
// screen.
//
// Properties preserved:
//   * Empty mirror lists overwrite previously-populated PSML lists —
...
//     matching PSML Field, deep-walking through Group / Optional /
//     Switch / Encrypted / Repeat-element (sub-agent C1 / H2).

import { isField } from "../utils";
import type { Container, Packet as PsmlPacket, Repeat } from "../types";
import type {
...
function mergeRepeat(c: Repeat, mirror: RendererPacket): Repeat {
  ...
  // import time (psmlToRenderer collapses them), so prefer the stripped
 ...
 *  Containers whose mirror state matches PSML state pass through
