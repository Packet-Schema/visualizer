// PSDL edit reducer — the canonical state machine for the Custom Packet
...
// inside a `PsdlPacket`. The grammar is defined and walked by
...
import type { Constraint, Container, Field, Packet, PsdlPacket } from "./types";
 ...
 * (e.g. turning a `repeat` into a `switch`) and corrupt the PSDL shape
...
export type EditAction =
  ...
  | { type: "replace-packet"; packet: PsdlPacket }
  | { type: "undo" }
  | { type: "redo" };
 ...
 */
function commit(state: EditState, next: PsdlPacket): EditState {
...
function wrapNode(
  ...
  switch (kind) {
    ...
    case "struct":
      // A Struct is not a Container in PSDL — we model 'wrap as struct' as a
...
export function makeInitialState(packet: PsdlPacket): EditState {
...
// Re-export types so consumers can `import type { PsdlPacket } ...` indirectly.
export type { Packet, PsdlPacket };
