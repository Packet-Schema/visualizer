...
// PSDL packets created in the Custom Packet Studio. Keys are stored under the
...
function readRaw(): PresetMap {
  ...
  // Validate every entry with `validatePsdlPacket` (the same checker
  ...
  // CSRF) — can't get a malformed PsdlPacket into the renderer or the
  ...
  for (const [key, value] of Object.entries(
    ...
    try {
      validatePsdlPacket(value as PsdlPacket);
      out[key] = value as PsdlPacket;
    } catch {
...
export function loadCustomPresets(): Record<string, PsdlPacket> {
  return readRaw();
}
 ...
 */
export function saveCustomPreset(key: string, packet: PsdlPacket): void {
