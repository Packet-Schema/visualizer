// PSDL 0.3 — Packet Schema Definition Language.
// Normalize a PSDL Packet into a flat NormalizedField[] that the cell-layout
// algorithm in `web/lib/psdl/layout.ts` can consume directly.
//
...
export function typeBits(type: Type, env: PacketEnv, fieldId?: string): number {
  switch (type.kind) {
    ...
    case "berLength": {
      // PSDL 0.4 — width is dynamic; consult env override under the namespaced
...
function walkContainer(c: Container, path: string, state: WalkState): void {
  ...
  switch (c.kind) {
    ...
    case "optional": {
      // PSDL 0.4 — emit inner field iff `when` evaluates truthy in env. A
